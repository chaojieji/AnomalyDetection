...
classes = ['bottle', 'cable', 'capsule', 'carpet', 'grid',
           ...
           'tile', 'toothbrush', 'transistor', 'wood', 'zipper', 'stone']
...
class MVTecDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if x.endswith("png"):
            ...
            # Convert images to handleable forms
            png = Image.open(x)
            png.load()  # required for png.split()
            background = Image.new("RGB", png.size, (255, 255, 255))
            # 3 is the alpha channel
            lst_channels = png.split()
            # If the input graph is in 3 channels, skip it
            if len(lst_channels) > 3:
